    ...
    printPackageResults(): void {
        ...
        this.packageResults.forEach((packageResult, packageName) => {
            ...
            packageResult.groups.forEach((group, folderPath) => {
                console.log(
                    `  📁 ${folderPath} (${group.passedTests}/${group.totalTests} 성공)`,
                );
            ...
            const avgDuration =
                packageResult.totalTests > 0
                    ? packageResult.totalDuration / packageResult.totalTests
                    : 0;
            console.log(`  📊 평균 실행 시간: ${avgDuration.toFixed(2)}ms`);
        });

        // 전체 통계
        const totalTests = Array.from(this.packageResults.values()).reduce(
            (sum, pkg) => sum + pkg.totalTests,
            0,
        );
        const totalPassed = Array.from(this.packageResults.values()).reduce(
            (sum, pkg) => sum + pkg.passedTests,
            0,
        );
        const totalFailed = Array.from(this.packageResults.values()).reduce(
            (sum, pkg) => sum + pkg.failedTests,
            0,
        );

        console.log('\n' + '='.repeat(80));
        console.log(
            `📊 전체 통계: 총 ${totalTests}개 테스트 | 성공: ${totalPassed} | 실패: ${totalFailed}`,
        );
    }
...
export const measurePerformance = async <T>(
    ...
    try {
        ...
        if (duration > 10) {
            console.warn(
                `⚠️  ${packageName}/${folderPath}/${testName}: ${duration.toFixed(2)}ms (성능 개선 필요)`,
            );
        }
 ...
 */
export const extractPackageInfo = (
    filePath: string,
): { packageName: string; folderPath: string } => {
    ...
    const folderPath =
        pathParts.slice(packagesIndex + 2, -1).join('/') || 'root';
