...
enum TokenType {
    ...
    tokenize(): Token[] {
      while (this.position < this.input.length) {
        ...
        if (/[0-9]/.test(char) || 
          ...
          this.tokens.push(new Token(TokenType.NUMBER, value, this.position - value.length));
          continue;
        }
        ...
        if (/[A-Za-z]/.test(char)) {
          ...
          if (value === 'AND' || value === 'OR') {
            this.tokens.push(new Token(TokenType.LOGICAL_OPERATOR, value, this.position - value.length));
          } else {
            this.tokens.push(new Token(TokenType.IDENTIFIER, value, this.position - value.length));
          }
          continue;
        }
        ...
        // 등호 처리
        if (char === '=') {
          this.tokens.push(new Token(TokenType.EQUALS, char, this.position));
          this.position++;
          continue;
        }
        ...
        // 괄호 처리
        if (char === '(') {
          this.tokens.push(new Token(TokenType.LEFT_PAREN, char, this.position));
          this.position++;
          continue;
        }
        if (char === ')') {
          this.tokens.push(new Token(TokenType.RIGHT_PAREN, char, this.position));
          this.position++;
          continue;
        }
  ...
  class Parser {
    private position: number = 0;
    private sidCounter: number = 12345601;
